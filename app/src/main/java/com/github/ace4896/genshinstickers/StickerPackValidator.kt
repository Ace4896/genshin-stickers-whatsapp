...
import com.github.ace4896.genshinstickers.StickerPackLoader.fetchStickerAsset
import java.io.IOException
import java.net.MalformedURLException
import java.net.URL

internal object StickerPackValidator {
    ...
    fun verifyStickerPackValidity(context: Context, stickerPack: StickerPack) {
        ...
        if (!TextUtils.isEmpty(stickerPack.licenseAgreementWebsite) && !isValidWebsiteUrl(
        ...
        if (!TextUtils.isEmpty(stickerPack.publisherEmail) && !Patterns.EMAIL_ADDRESS.matcher(
        ...
        try {
            val stickerAssetBytes = fetchStickerAsset(
                stickerPack.identifier,
                stickerPack.trayImageFile!!,
                context.contentResolver
            )
            if (stickerAssetBytes.size > TRAY_IMAGE_FILE_SIZE_MAX_KB * KB_IN_BYTES) {
            ...
            val bitmap = BitmapFactory.decodeByteArray(stickerAssetBytes, 0, stickerAssetBytes.size)
            if (bitmap.getHeight() > TRAY_IMAGE_DIMENSION_MAX || bitmap.getHeight() < TRAY_IMAGE_DIMENSION_MIN) {
                throw IllegalStateException("tray image height should between " + TRAY_IMAGE_DIMENSION_MIN + " and " + TRAY_IMAGE_DIMENSION_MAX + " pixels, current tray image height is " + bitmap.getHeight() + ", tray image file: " + stickerPack.trayImageFile)
            }
            if (bitmap.getWidth() > TRAY_IMAGE_DIMENSION_MAX || bitmap.getWidth() < TRAY_IMAGE_DIMENSION_MIN) {
                throw IllegalStateException("tray image width should be between " + TRAY_IMAGE_DIMENSION_MIN + " and " + TRAY_IMAGE_DIMENSION_MAX + " pixels, current tray image width is " + bitmap.getWidth() + ", tray image file: " + stickerPack.trayImageFile)
            }
        } catch (e: IOException) {
    ...
    private fun validateSticker(
        ...
        if (sticker.emojis.size < EMOJI_MIN_LIMIT) {
            throw IllegalStateException("To provide best user experience, please associate at least 1 emoji to this sticker, sticker pack identifier: " + identifier + ", filename: " + sticker.imageFileName)
        }
    ...
    private fun validateStickerFile(
        ...
        try {
            val stickerInBytes = fetchStickerAsset(identifier, fileName, context.contentResolver)
            ...
            try {
                ...
                if (webPImage.height != IMAGE_HEIGHT) {
                    throw IllegalStateException("sticker height should be " + IMAGE_HEIGHT + ", current height is " + webPImage.height + ", sticker pack identifier: " + identifier + ", filename: " + fileName)
                }
                if (webPImage.width != IMAGE_WIDTH) {
                    throw IllegalStateException("sticker width should be " + IMAGE_WIDTH + ", current width is " + webPImage.width + ", sticker pack identifier: " + identifier + ", filename: " + fileName)
                }
                if (animatedStickerPack) {
                    ...
                    if (webPImage.duration > ANIMATED_STICKER_TOTAL_DURATION_MAX) {
                        throw IllegalStateException("sticker animation max duration is: " + ANIMATED_STICKER_TOTAL_DURATION_MAX + " ms, current duration is: " + webPImage.duration + " ms, sticker pack identifier: " + identifier + ", filename: " + fileName)
                    }
                } else if (webPImage.frameCount > 1) {
